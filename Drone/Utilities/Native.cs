...
public static class Native
{
    ...
    public static IntPtr NtAllocateVirtualMemory(IntPtr hProcess, int regionSize, MEMORY_ALLOCATION allocation,
        MEMORY_PROTECTION protection)
    {
        var baseAddress = IntPtr.Zero;
        var size = new IntPtr(regionSize);
        
        object[] parameters =
        {
            hProcess, baseAddress, IntPtr.Zero, size,
            (uint)allocation, (uint)protection
        };
    ...
    public static bool NtWriteVirtualMemory(IntPtr hProcess, IntPtr baseAddress, byte[] buffer)
    {
        ...
        object[] parameters =
        {
            hProcess, baseAddress, buf, (uint)buffer.Length, bytesWritten
        };
        ...
        return bytesWritten == (uint)buffer.Length;
    }

    public static void NtFreeVirtualMemory(IntPtr hProcess, IntPtr baseAddress, int regionSize)
    {
        var size = new IntPtr(regionSize);
        
        object[] parameters =
        {
            hProcess, baseAddress, size, (uint)MEMORY_ALLOCATION.MEM_RELEASE
        };
        
        Generic.DynamicApiInvoke(
            "ntdll.dll",
            "NtFreeVirtualMemory",
            typeof(Delegates.NtFreeVirtualMemory),
            ref parameters);
    }

    public static bool NtProtectVirtualMemory(IntPtr hProcess, IntPtr baseAddress, int regionSize,
        MEMORY_PROTECTION newProtection, out MEMORY_PROTECTION oldProtection)
    {
        var size = new IntPtr(regionSize);

        object[] parameters =
        {
            hProcess, baseAddress, size, (uint)newProtection, (uint)0
        };
        
        var status = (uint)Generic.DynamicApiInvoke(
            "ntdll.dll",
            "NtProtectVirtualMemory",
            typeof(Delegates.NtProtectVirtualMemory),
            ref parameters);
    ...
    public static IntPtr NtCreateThreadEx(IntPtr hProcess, IntPtr startAddress)
    {
        var hThread = IntPtr.Zero;
        
        object[] parameters =
        {
            hThread, DInvoke.Data.Win32.WinNT.ACCESS_MASK.MAXIMUM_ALLOWED, IntPtr.Zero, hProcess,
            startAddress, IntPtr.Zero, false, 0, 0, 0, IntPtr.Zero
        };
    ...
    public static bool NtResumeThread(IntPtr hThread)
    {
        object[] parameters =
        {
            hThread, (uint)0
        ...
        var status = (uint)Generic.DynamicApiInvoke(
            "ntdll.dll",
            "NtResumeThread",
            typeof(Delegates.NtResumeThread),
            ref parameters);
